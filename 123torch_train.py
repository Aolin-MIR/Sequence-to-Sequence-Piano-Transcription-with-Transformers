...
total_examples = 1300295
train_path = "ours_train.tfrecord"
valid_path = 'valid.tfrecord'
train_path = "/mnt/data/aug_127_train.tfrecord.merge"
valid_path = "/mnt/data/aug_127_valid.tfrecord.merge"
batch_size = 32
# seg_width=16
seg_width = 127
epochs = 40
...
def note_4f1_loss(evalPrediction):
    ...
    for y_pred, y_true in zip(evalPrediction.predictions, evalPrediction.label_ids):
        ...
        y_pred = [((x-1)//88, (x-1) % 88)
                  for x in y_pred if x != seg_width*88+1]
...
def parse_fn(features):
    ...
    if features['inputs_embeds'].shape[-1] not in [512, nbins]:
        ...
        assert 1 == 2
    # for x in features['labels']:
    #     if x > seg_width*88:
    #         print(119,'!!!!!!!',features['labels'])
...
class MyTrainer(Seq2SeqTrainer):
    ...
    def prediction_step(
        ...
        # else:
        generation_inputs = {"input_ids": None,
...
args = Seq2SeqTrainingArguments(output_dir='./output', overwrite_output_dir=False, do_train=True, do_eval=True, do_predict=False, evaluation_strategy='steps', prediction_loss_only=False, per_device_train_batch_size=batch_size, per_device_eval_batch_size=int(batch_size/beams), per_gpu_train_batch_size=batch_size, per_gpu_eval_batch_size=int(batch_size/beams), gradient_accumulation_steps=1, eval_accumulation_steps=200, max_grad_norm=1.0,  max_steps=int(train_ex*epochs/batch_size),  log_level='passive', log_level_replica='passive', log_on_each_node=True,
                                logging_dir=None, logging_strategy='steps', logging_first_step=True,
                                logging_steps=int(train_ex/batch_size),
                                logging_nan_inf_filter=True, save_strategy='steps', save_steps=int(train_ex/batch_size), save_total_limit=10, save_on_each_node=False, no_cuda=False, seed=42, fp16=False, fp16_opt_level='O1', fp16_backend='auto', fp16_full_eval=False, local_rank=- 1, xpu_backend=None, tpu_num_cores=None, tpu_metrics_debug=False, dataloader_drop_last=False,
                                eval_steps=int(train_ex/batch_size),
                                dataloader_num_workers=4, generation_max_length=seg_width*2,
                                predict_with_generate=True,
                                generation_num_beams=beams,
                                learning_rate=5e-5, weight_decay=0.0, adam_beta1=0.99, adam_beta2=0.999, adam_epsilon=1e-08
                                )

optimizer = transformers.AdamW(model.parameters(), 1e-5)
